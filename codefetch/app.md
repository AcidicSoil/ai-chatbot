...
app/(chat)/api/lmstudio/embed/route.ts
```
import { auth } from "@/app/(auth)/auth";
import { ChatSDKError } from "@/lib/errors";
import {
...
app/(chat)/api/lmstudio/load/route.ts
```
// app/api/lmstudio/load/route.ts
import { NextRequest, NextResponse } from "next/server";
import { getLMStudioClient } from "@/lib/lmstudio/client";
...
app/(chat)/api/lmstudio/unload/route.ts
```
...
app/(chat)/api/lmstudio/status/route.ts
```
